                                ...
                                login = Sys.getenv(c("AQUSER", "AQPASS")))
{
  ...
  plot <- ggplot2::ggplot() +
    ...
    ggplot2::geom_path(data = plotdf, ggplot2::aes(x = timestamp_MST, y = plyr::round_any(max(na.omit(value)), 0.5, f = ceiling), colour = factor(grade_description), group = 1), size = 2.5, show.legend = FALSE) +
  ...
  print("Grade key, and data .csv written")
