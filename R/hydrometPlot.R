...
#' @param startDay The start day of year for the plot x-axis. Can be specified as a number from 1 to 365, as a character string of form "yyyy-mm-dd", or as a date object. Either way the day of year is the only portion used, specify years to plot under parameter `years`. Not currently in use for discrete data types.
#' @param endDay The end day of year for the plot x-axis. As per `startDay`. Not currently in use for discrete data types.
                         ...
                         dbPath = "default")
{
  ...
  type <- tolower(type)
  if (type == "discrete" & (startDay != 1 | endDay != 365)){
    print("hydrometPlot does not currently support user-specified start and end days. Your inputs have been modified to the function defaults.")
    startDay <- 1
    endDay <- 365
  }
  ...
  tryCatch({
    ...
    # } else {
    #   endDay <<- as.POSIXct(as.numeric(endDay)*60*60*24, origin = paste0(last_year, "-12-31 23:59:59"), tz = "UTC")
    #   endDay <<- lubridate::force_tz(endDay, tzone)
    # }
  })
  ...
  if (type == "continuous"){
    ...
    for (i in rev(years)){ #Using rev so that the most recent year gets realtime, if possible
      ...
      if (length(day_seq) < 60){
        ...
        new_realtime <- utils::tail(new_realtime, 20000)
    ...
    ribbon <- data.frame()
    ribbon_seq <- seq.Date(min(as.Date(day_seq)), max(as.Date(day_seq))+1, by = "day")
    ...
    if (overlaps){ # This section sorts out the overlap years, builds the plotting column
      temp <- data.frame(date = day_seq,
                         year = lubridate::year(day_seq),
                         month = lubridate::month(day_seq),
                         day = lubridate::day(day_seq))
      temp$day <- stringr::str_pad(temp$day, 2, side = "left", pad = "0")

      temp$md <- paste0(temp$month, temp$day)
      temp$md <- as.numeric(temp$md)
      temp <- temp[!(temp$year == max(temp$year)), ]
  ...
  if (type == "continuous"){
    ...
    legend_length <- length(unique(stats::na.omit(realtime$plot_year)))
    ...
    plot <- plot +
      ggplot2::geom_line(ggplot2::aes(colour = as.factor(plot_year)), linewidth = line_size, na.rm = T) +
      ggplot2::scale_colour_manual(name = "Year", labels = rev(unique(realtime$plot_year)), values = colours[1:legend_length], na.translate = FALSE, breaks=rev(unique(stats::na.omit(realtime$plot_year))[1:legend_length]))
  ...
  #Make the discrete plots --------------------
  if (type == "discrete") {
    legend_length = length(years)
    plot <- ggplot2::ggplot(all_discrete, ggplot2::aes(x = fake_date, y = value, group = fake_date)) +
      ggplot2::labs(x = "", y = if (parameter == "SWE") "SWE" else paste0(stringr::str_to_title(parameter), " (", units, ")")) +
      ggplot2::theme_classic() +
